        ...
        if wallets:
            ...
            for wallet in wallets:
                try:
                    ...
                    if 'wrong password or salt' in private_key:
                        logging.error(f'Wrong password or salt! Decrypt private key not possible')
                        sys_exit = True
                        break
                    ...
                    if re.match(r'\w' * 64, private_key):
                        ...
                        if wallet_instance and wallet_instance.name != name:
                            ...
                            client = Client(private_key=private_key, network=Networks.Ethereum,)# proxy=proxy)
                            address = client.account.address
                            mint_nft = random.randint(settings.mint_nft.from_, settings.mint_nft.to_)
                            swaps = random.randint(settings.swaps.from_, settings.swaps.to_)
                            liquidity = random.randint(settings.liquidity.from_, settings.liquidity.to_)
                            lending = random.randint(settings.lending.from_, settings.lending.to_)
                            dmail = random.randint(settings.dmail.from_, settings.dmail.to_)
                            wallet_instance = Wallet(
                                private_key=wallet['private_key'], address=address, name=name,
                                proxy=proxy, mint_nft=mint_nft, swaps=swaps, liquidity=liquidity, lending=lending, dmail=dmail
                            )
                            db.insert(wallet_instance)
                            imported.append(wallet_instance)

                except:
                    logging.exception('Import.wallets')
            ...
            if imported:
                text += (f'\n--- Imported\nN\t{"address":<72}{"name":<16}'
                         f'{"liquidity":<15}{"swaps":<10}{"dmail":<10}{"shit_nft":<10}')
                for i, wallet in enumerate(imported):
                    text += (f'\n{i + 1:<8}{wallet.address:<72}{wallet.name:<16}'
                             f'{wallet.liquidity:<15}{wallet.swaps:<10}\t{wallet.dmail:<10}\t{wallet.mint_nft:<10}')
